var thisJs = (function() {
    ...
    return {
        ...
        getRecords: function() {
            ...
            data_table = table = $dataTable.DataTable({
                        ...
                        if(editPermission == 1){
                            $('.btn-toolbar').append(
                                ...
                                `<div class="input-group-prepend">
                                    <button type="button" class="btn btn-danger dropdown-toggle dt-btn-payment-status" data-toggle="dropdown" disabled>
                                    Status
                                    </button>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item dt-payment-process" href="#" data-status="PROCESS" onclick="thisJs.changePaymentStatus(event)">Process</a>
                                    </div>
                                </div>`
                            );
                        }
                ...
                columnDefs: [
                    {
                        ...
                        render: function(e, a, t, n) {
                            if(viewPermission == 1 && editPermission == 1 && t.payment_status == 'PENDING'){
                ...
                columns: [
                    ...
                    { data: "order_number", name: "order_number", sortable: true, sWidth: '13%' },
                    { data: "order_date", name: "order_date", sortable: true, sWidth: '13%' },
                    { data: "book_name", name: "book_name", sortable: true, sWidth: '20%' },
                    { data: "author_name", name: "author_name", sortable: true, sWidth: '20%' },
                    { data: "royalty_amount", name: "royalty_amount", sortable: true, sWidth: '10%' },
                    { data: "payment_status_badge", name: "payment_status_badge", sortable: true, className:"text-center", sWidth: '21%' }
                ],
            ...
            $dataTable
                ...
                .on("click", 'input[type="checkbox"]', function(e) {
                    ...
                    if (
                        ...
                        $(".dt-btn-payment-status").prop("disabled", false);
                    } else {
                        $(".dt-btn-payment-status").prop("disabled", true);
                    }
        ...
        changePaymentStatus: function(e) {
            ...
            var paymentStatus = currentElement.dataset.status;
            ...
            Swal.fire({
                ...
                confirmButtonText: "Yes, change payment status!"
                }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: $dataTable.data("payment-status-url"),
                        data: { ids: ids, payment_status: paymentStatus },
                        beforeSend: function() {
                            $(".dt-btn-payment-status").prop("disabled", true);
                        },
                        success: function(response) {
                            ...
                            if (data_table.data().length == 0) {
                                $(".dt-btn-payment-status").attr("disabled", "disabled");
                            }
                            else {
                                $(".dt-btn-payment-status").removeAttr("disabled");
                            }
                        },
                        error: function() {},
                        complete: function() {
