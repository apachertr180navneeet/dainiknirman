            ...
            $form.validate({
                ...
                rules: {
                    ...
                    'category_group[]': {
                        required: {
                            depends: function(){
                                return ($("input[name='is_grouped']:checked").val() == 1 && $("#category_group option[selected]").length == 0);
                            }
                        }
                    },
                    'size_category[]': {
                        required: {
                            depends: function(){
                                return ($("#parent_id").val() == '' && $("#size_category option[selected]").length == 0);
                            }
                        }
                    },
                    image: {
                        required: {
                            depends: function(){
                                if(($("#image").attr("data-default-file") === undefined || $("#image").attr("data-default-file") == '') && $("#parent_id").val() == ''){
                                    return true;
                                }
                            }
                        },
