        ...
        <div class="row">
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <h3 class="card-title">Contest Authors</h3>
                        </div>
                        <div class="col-md-5">
                        </div>
                        <div class="col-md-1">
                            @if(auth()->user()->hasPermissionTo('Add Contest Author'))
                                {{-- <a href="{{ route('admin.contest-authors.create') }}" class="btn btn-block btn-primary"><i class="fas fa-plus"></i> Add</a> --}}
                            @endif
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card-body" style="overflow: auto;">
                        <table id="dataTable" class="table table-bordered table-hover" data-url="{{route('admin.contest-authors.getContestAuthors', ['contest_id' => $contestId])}}" data-destroy-url="{{route('admin.contest-authors.destroy')}}" data-change-status-url="{{route('admin.contest-authors.changeStatus')}}" data-export-csv-url="{{route('admin.contest-authors.export', ['type' => 'excel'])}}">
                            <thead>
                                <tr>
                                    <th>S.no.</th>
                                    <th>Contest Title</th>
                                    <th>User Contest Title</th>
                                    <th>Contest Date</th>
                                    <th>Author Name</th>
                                    <th>Result</th>
                                    <th>Date</th>
...
<!-- /.content -->
@php
$viewPermission = auth()->user()->hasPermissionTo('View Contest Author');
$editPermission = auth()->user()->hasPermissionTo('Edit Contest Author');
$deletePermission = auth()->user()->hasPermissionTo('Delete Contest Author');
@endphp
...
<script src="{{ asset('public/js/contest-authors/contest-authors.js') }}"></script>
@endpush
