            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-1">
                            @if(auth()->user()->hasPermissionTo('Add Subscription'))
                                <a href="{{ route('admin.subscriptions.create') }}" class="btn btn-block btn-primary"><i class="fas fa-plus"></i> Add</a>
                            @endif
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="card-body" style="overflow: auto;">
                        <table id="dataTable" class="table table-bordered table-hover" data-url="{{route('admin.subscriptions.getSubscriptions')}}" data-destroy-url="{{route('admin.subscriptions.destroy')}}" data-change-status-url="{{route('admin.subscriptions.changeStatus')}}" data-export-csv-url="{{route('admin.subscriptions.export', ['type' => 'excel'])}}">
                            <thead>
                                <tr>
                                    <th>S.no.</th>
                                    <th>Name</th>
...
<!-- /.content -->
@php
$viewPermission = auth()->user()->hasPermissionTo('View Subscription');
$editPermission = auth()->user()->hasPermissionTo('Edit Subscription');
$deletePermission = auth()->user()->hasPermissionTo('Delete Subscription');
@endphp
...
<script src="{{ asset('public/js/subscriptions/subscriptions.js') }}"></script>
@endpush
