...
class UserController extends Controller
{
    public function index()
    {
        ...
        // Send view data
        $showItemsPerPage = getSettingBySlug('show-items-per-page');
        $this->viewData['pageTitle'] = 'User';
        $this->viewData['breadcrumb'] = $breadcrumb;
        $this->viewData['showItemsPerPage'] = $showItemsPerPage;
    ...
    public function getUsers(Request $request)
    {
        ...
        if(count($records) > 0)
        {
            foreach($records as $key => $value)
            {
                ...
                $status = '';
                $action = '';
                $pendingRoyaltyAmount = $value->pending_royalty;

                $name = $value->name ?? $name;
                
                if(!empty($value->role)){
                    $name .= '<br/><small class="badge badge-info">'.$value->role->name.'</small>';
                }

                if(!empty($pendingRoyaltyAmount)){
                    $name .= '<br/><a href="'.route('admin.royalties.index').'?author='.$value->id.'"><small class="badge badge-danger">Pending: <i class="fa fa-rupee-sign"></i>'.$pendingRoyaltyAmount.'</small></a>';
                }
                ...
                if(isset($value->active_plan) && !empty($value->active_plan))
                {
                    $arrSubscriptionDetail = json_decode($value->active_plan->subscription_details);
                    $subscriptionName = $arrSubscriptionDetail->name;
                    $subscriptionStartDate = date("d-m-Y", strtotime($value->active_plan->start_date)).' at '.date("H:i", strtotime($value->active_plan->start_date));
                    $subscriptionEndDate = date("d-m-Y", strtotime($value->active_plan->end_date)).' at '.date("H:i", strtotime($value->active_plan->end_date));
                }
                ...
                $arr_data[] = array(
                    ...
                    "created" => $created,
                    "action" => $action,
                    "pendingRoyaltyAmount" => $pendingRoyaltyAmount
                );
            }
        }
    ...
    public function create()
    {
        ...
        // Get Roles list
        $roles = Role::select('id', 'name')
        ->whereNotIn('id', [config('constants.roles.ADMIN.value')])
        ->get();
