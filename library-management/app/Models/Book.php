...
class Book extends Model
{
    ...
    /* public function scopeGetAnthologyWriteups($model, $limit = null, $offset = null, $search = null, $filter = array(), $sort = array())
    {
        $records = Book::select('books.id', 'books.book_name', 'books.author_name', 'books.launch_date', 'books.book_type', 'books.book_category', 'books.price', 'books.cover_picture', 'books.book_pdf', 'books.status', 'books.created_at', DB::raw("author_royalties.royalty as author_royalty, sum(app_royalties.royalty) as app_royalty"))
        ->leftJoin(DB::raw("(select r.book_id, sum(r.author_royalty) as royalty from royalties r group by r.book_id) as author_royalties"), function($join){
            $join->on("author_royalties.book_id", "=", "books.id");
        })
        ->leftJoin(DB::raw("(select r.book_id, sum(distinct r.app_royalty) as royalty from royalties r group by r.book_id, r.order_id) as app_royalties"), function($join){
            $join->on("app_royalties.book_id", "=", "books.id");
        })
        ->where(function($query){
            //
        })
    ...
    public function scopeGetAnthologyWriteups($model, $limit = null, $offset = null, $search = null, $filter = array(), $sort = array())
    {
        $records = Book::select('books.id', 'books.book_name', 'books.author_name', 'books.launch_date', 'books.book_type', 'books.book_category', 'books.price', 'books.cover_picture', 'books.book_pdf', 'books.status', 'books.created_at', DB::raw("author_royalties.royalty as author_royalty, sum(app_royalties.royalty) as app_royalty"), DB::raw("GROUP_CONCAT(distinct users.mobile separator ', ') as user_mobile"))
        ->leftJoin(DB::raw("(select r.book_id, sum(r.author_royalty) as royalty from royalties r group by r.book_id) as author_royalties"), function($join){
            $join->on("author_royalties.book_id", "=", "books.id");
        })
        ->leftJoin(DB::raw("(select r.book_id, sum(distinct r.app_royalty) as royalty from royalties r group by r.book_id, r.order_id) as app_royalties"), function($join){
        ...
        // ->leftJoin(DB::raw("(select id, order_id, book_id, author_id from royalties r group by r.book_id, r.author_id) as authors"), function($join){
        //     $join->on("authors.book_id", "=", "books.id");
        // })
        // ->leftJoin('users', 'users.id', '=', 'authors.author_id')
        ->leftJoin('book_category_authors', 'book_category_authors.book_id', '=', 'books.id')
        ->leftJoin('users', 'users.id', '=', 'book_category_authors.author_id')
        ->where(function($query){
            //
        })
        ->where(function($query) use($search, $filter, $sort){
        ...
        if((!(empty($sort)) && $sort['column'] >= 0) || !empty($search))
        {
            $arr_fields = array(
                // "", 
                DB::raw("GROUP_CONCAT(distinct users.mobile separator ', ')"),
                "books.book_name",
                "books.book_category",
                "books.price",
                "books.launch_date",
